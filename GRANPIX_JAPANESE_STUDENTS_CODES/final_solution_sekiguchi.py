...
def wall_follow2(direction):
    ...
    #if (left_dist > 9999 or right_dist > 9999) and panic < 100:
...
def line_follow(color_image):
    ...
    if contour is not None:
        ...
        # Display the image to the screen
        rc.display.show_color_image(cropped_image)
        return LINE_FOLLOW_SPEED, rc_utils.remap_range(center[1], 0, rc.camera.get_width(), -1, 1)
...
def iine_follow_2():
    image = rc.camera.get_color_image()
    image = rc_utils.crop(image, (0, 0), (rc.camera.get_height(), rc.camera.get_width() // 2 ))
    if image is None:
        print("No image")
        rc.drive.stop()
        return
    contours = rc_utils.find_contours(image, ORANGE[0], ORANGE[1]) + rc_utils.find_contours(image, PURPLE[0],PURPLE[1])
    contour = rc_utils.get_largest_contour(contours, MIN_CONTOUR_AREA)
    ...
    if contour is not None:
        ...
        print(((contour_center[1] - rc.camera.get_width() / 2) + 100) / 320)
        return 0.8 , ((contour_center[1] - rc.camera.get_width() / 2) + 170) / 320
    else:
        return 0.5 ,1

def wall_follow3(direction):
    """
    Determines the angle of the wheels necessary to follow the wall on the side
    specified by the direction parameter.
    ...
    print(right_dist,";",left_dist,";",angle)
