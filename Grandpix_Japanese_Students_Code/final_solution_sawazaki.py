...
from lab2b_solution_toko import ORANGE
import sys
import cv2 as cv
import numpy as np
import enum
...
def update():
    ...
    if cur_stage == Stage.none:
    ...
    elif cur_stage == Stage.marker_id or cur_stage == Stage.wall:
        # After the first two markers, follow the wall indicated by the marker
        speed, angle = wall_follow(cur_direction)#wall_follow2(cur_direction)
        ...
        if angle is None:
            _, angle = wall_follow(cur_direction)#2
    ...
    if cur_stage == Stage.lane:
        speed *= 0.75
        angle *= 1.1
    elif cur_stage == Stage.orientation:
        speed *= 0.3
        angle *= 1.6
    elif cur_stage != Stage.cone and cur_stage != Stage.wall and cur_stage != Stage.color:
        if speed > 0:
            speed  = (speed * 100) ** 3 / 300
            angle *= 0.9
        else:
            speed *= 2
    ...
    lidar_left =  rc_utils.get_lidar_average_distance(
        lidar, 270, 300
    )
    lidar_right =  rc_utils.get_lidar_average_distance(
        lidar, 90, 60
    )

    frame_speed = (prev_distance2 - distance) / rc.get_delta_time()
    cur_speed += ALPHA * (frame_speed - cur_speed)
    prev_distance2 = distance
    print(f"\ncurspeed : {cur_speed}\nframespeed : {frame_speed}\n")

    #stuck
    if -10 < cur_speed < 10 or center_distance < 15 or lidar_left < 10 or lidar_right < 10 or distance < 15:# and cur_stage != None:
        print("stack?")
        stuck_time += rc.get_delta_time()
        if stuck_time > 6:
            back = True
    else:
        stuck_time = 0
    ...
    if back == True and cur_stage != Stage.cone and cur_stage != Stage.lane:
        back_count += 1
        if back_count < 20:
            speed = -1
            angle *= -1.5
            #back_count += 1#rc.get_delta_time()
            print("back!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        #elif back_count < 30:
        #    if speed > 0:
        #        angle *= -1
...
def change_stage(new_marker, color_image):
    ...
    if new_marker.get_color() == "red":
        ...
        cur_color = ORANGE
        #if cur_marker.get_id() == 1:
            #primary_color = [PURPLE, ORANGE]
            #secondary_color = ORANGE
        colors_lane = [PURPLE, ORANGE]
    elif new_marker.get_color() == "purple":
        cur_color = PURPLE
        #if cur_marker.get_id() == 1:
            #primary_color = [ORANGE, PURPLE]
            #secondary_color = PURPLE
        colors_lane = [ORANGE, PURPLE]
    else:
        cur_color = PURPLE
...
def wall_follow2(direction):
    ...
    global IS_WALL_DISTANCE
    global S_DISTANCE
    global TURN_DISTANCE


    if cur_stage == Stage.wall: 
        WALL_DISTANCE = 36
        IS_WALL_DISTANCE = 80#70
        S_DISTANCE = 50
        TURN_DISTANCE = 70
        COLLISION_DISTANCE = 35#30
